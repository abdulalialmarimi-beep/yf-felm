    ...
    if (cmd === "تشغيل") {
      ...
      if (dashIndex === -1) {
        return message.reply(
          "❌ الصيغة الصحيحة:\n`" + PREFIX + "تشغيل اسم الفيلم - رابط`\n\nمثال:\n`" + PREFIX + "تشغيل فروزن - https://youtu.be/xxx`"
        );
      }

      const displayName = fullText.slice(0, dashIndex).trim();
      const rawUrl = fullText.slice(dashIndex + 3).trim();

      if (!displayName || !rawUrl) {
        return message.reply("❌ تأكد من كتابة الاسم والرابط بشكل صحيح.");
      }
      ...
      await message.reply(`⏳ جاري تجهيز: **${displayName}**${isYT ? " (يوتيوب...)" : ""}...`);
      ...
      message.reply(
        [
          "**🎬 أوامر بوت السينيما:**",
          `\`${PREFIX}تشغيل اسم - رابط\` — تشغيل فيديو (يوتيوب أو mp4 مباشر)`,
          `\`${PREFIX}ايقاف\` — إيقاف التشغيل والخروج`,
          "",
          "**أمثلة:**",
          `\`${PREFIX}تشغيل فروزن - https://youtu.be/xxx\``,
          `\`${PREFIX}تشغيل فيلم - https://example.com/video.mp4\``,
        ].join("\n")
      );
    }
  } catch (err) {
