...
streamer.client.on("messageCreate", async (message) => {
  ...
  try {
    ...
    if (cmd === "ادخل") {
      ...
      if (!channelId) {
        return message.reply("❌ حط ID قناة الصوت.\nمثال: `!ادخل 123456789`");
      }
      ...
      // يدخل الصوت فوراً ويفتح Go Live بشاشة سوداء
      await streamer.joinVoice(guild.id, channelId);

      // فتح Go Live بفيديو وهمي (شاشة سوداء)
      const silentVideoUrl = "https://commondatastorage.googleapis.com/gtv-videos-bucket/sample/ForBiggerEscapes.mp4";
      const ffmpegOptions = {
        frameRate: 1,
        bitrateVideo: 100,
        bitrateVideoMax: 200,
      ...
      try {
        const { output } = prepareStream(silentVideoUrl, ffmpegOptions);
        playStream(output, streamer, { type: "go-live" }).catch(() => {});
      } catch {}

      await message.reply(`✅ دخلت القناة وفتحت الشاشة!\nالحين اكتب:\n\`!تشغيل رابط\``);
    ...
    // ===== أمر تشغيل =====
      ...
      if (!rawUrl) {
        return message.reply("❌ حط الرابط.\nمثال: `!تشغيل https://example.com/video.mp4`");
      }

      if (!rawUrl.startsWith("http")) {
        return message.reply("❌ الرابط غير صحيح.");
      }
      ...
      if (!voiceChannel) {
        return message.reply("❌ ما قدرت أوصل للقناة. جرب `!ادخل ID` مرة ثانية.");
      }
    ...
    // ===== أمر مساعدة =====
      ...
      message.reply([
        "**🎬 أوامر بوت السينيما:**",
        "",
        "`!ادخل ID` — يدخل قناة الصوت ويفتح الشاشة",
        "`!تشغيل رابط` — يشغل الفيديو",
        "`!ايقاف` — يوقف ويخرج",
        "",
        "**مثال:**",
        "`!ادخل 1234567890`",
        "`!تشغيل https://example.com/video.mp4`",
      ].join("\n"));
    }
